...
#include "Algorithms/TrafficAssignment/TraversalCostFunctions/DecreasingFunction/InverseFunction.h"
#include "Algorithms/TrafficAssignment/TraversalCostFunctions/DecreasingFunction/VehiclesFunction.h"
#include "Algorithms/TrafficAssignment/TraversalCostFunctions/DecreasingFunction/DelayedFunction.h"
...
inline void assignTraffic(const CommandLineParser& clp) {
    ...
    auto statFileName = fileName + "_info_" + "stat";
    ...
    //Write the graph tp txt.file



    std::ofstream vertexInfo;
    std::ofstream edgeInfo;
    auto graphInfoFile = fileName + "_GraphInfo";
    vertexInfo.open(graphInfoFile + "_vertexInfo.json");
    edgeInfo.open(graphInfoFile + "_edgeInfo.json");

    if (vertexInfo.is_open() && edgeInfo.is_open()) {
        vertexInfo<<"{\"type\" : \"MultiPoint\" ," << std::endl;
        vertexInfo<< "\"coordinates\" : " << std::endl;
        vertexInfo<<"[";


        edgeInfo<<"{\"type\" : \"MultiLineString\" ," << std::endl;
        edgeInfo<< "\"coordinates\" : " << std::endl;
        edgeInfo<<"[";

        for (int u = 0; u < graph.numVertices() - 1 ; u++) {
            vertexInfo << "[" << graph.LatLngAttribute::latLng(u).lngInDeg() << "," << graph.LatLngAttribute::latLng(u).latInDeg() << "],";
        ...
        vertexInfo << "[" << graph.LatLngAttribute::latLng(last).lngInDeg() << "," << graph.LatLngAttribute::latLng(last).latInDeg() << "]";
        vertexInfo << "]" ;
    ...
    if (!statFileName.empty()) {
        ...
        if (traversalCostFunction == "inverse")
            statFile << "# alpha : " << alpha <<" beta : " <<beta << " scale : " << scale << "\n";
    ...
    FWAssignmentT fwAssignment(graph, odPairs, verbose, alpha, beta, scale);
    if (statFile.is_open()) {
...
void chooseTraversalCostFunction(const CommandLineParser& clp) {
    ...
    else if (func == "vehicles")
        chooseShortestPathAlgo<ObjFunctionT, VehiclesFunction>(clp);
    else if (func == "delayed")
        chooseShortestPathAlgo<ObjFunctionT, DelayedFunction>(clp);
    else
